...
public class WordCruncher {
    ...
    public static void main(String[] args) {
        ...
        for (String subWord : words) {
            occurrences.putIfAbsent(subWord, 0);
            occurrences.put(subWord, occurrences.get(subWord) + 1);
            int index = target.indexOf(subWord);
            while (index != -1) {
                table.putIfAbsent(index, new ArrayList<>());
                table.get(index).add(subWord);
                index = target.indexOf(subWord, index + 1);
            }
        }
        permute(0);
