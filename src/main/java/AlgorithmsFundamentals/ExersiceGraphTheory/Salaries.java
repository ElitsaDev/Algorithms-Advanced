...
public class Salaries {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < employees; i++) {
            ...
            for (int emp = 0; emp < line.length(); emp++) {
                ...
                if (letter == 'Y') {
                    managersCount[emp]++; //преброяваме за всеки ред от матрицата, колко Y има, тоест колко мениджъра
                    graph.get(i).add(emp);
                }
            }
        }
        ...
        for (int i = 0; i < managersCount.length; i++) {
            if (managersCount[i] == 0) { //намираме един мениджър, който няма шефове над себе си
                sources.add(i);
            }
        }
    ...
    private static void dfs(int node) {
        ...
        //за big-boss - той няма родители и няма да влезем в горния цикъл
